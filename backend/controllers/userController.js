...
const passwordRegex = /^(?=.*[a-zA-Z])(?=.*\d)(?=.*[!@#$%^])[a-zA-Z\d!@#$%^&*]{3,30}$/


const getAllUsers = asyncHandler(async(req,res) => {
    const user = await User.find().select('-password -bankAccount -notifications').lean()
    res.json({user})
})
...
const createNewUser = asyncHandler(async (req, res) => {
    ...
    if( !userNameRegex.test(username)){
        return res.status(409).json({ message: 'check that username contains characters a-z or A-Z and less than 15 characters' })
    }

     // date validation : (2) check if username has characters only a-zA-Z and length is < 15
     if( !passwordRegex.test(password)){
        return res.status(409).json({ message: 'check password. Can contains numbers, characters, or special symbols like "!@#$%^&*". Between 3-30 characters ' })
     }
    ...
    if(user){
        ...
        return res.status(500).json({ message: `Server error... check later please: ${username}`})
    }
...
const updateUser = asyncHandler(async (req, res) => {
    ...
    res.status(200).json({message:`user ${updatedUser.username} sucessfully updated`})
